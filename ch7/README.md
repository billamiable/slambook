...
由于出现了误匹配，在构建优化问题时会将某个误差项给的数据错误地加入到图优化模型中。但是优化算法并不能辨别出这是个错误数据，它会把所有的数据都当做误差来处理。这时，算法会看到一条误差很大的边，它的梯度也很大，算法就会试图调整与它相关的变量来使得目标函数下降更多。这往往会抹平了其他正确边的影响，使优化算法专注于调整一个错误的值。

避免误匹配的方法：基于RANSAC的单应性矩阵和本质矩阵求解来去除误匹配、设计鲁棒核函数、显式地将内点和外点进行区别并进行概率建模等。
